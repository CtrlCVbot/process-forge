...
**model 필드** - 모델 선택:
```yaml
model: opus  # 최고 성능 (권장 기본값)
model: sonnet  # 비용 절감 필요 시
model: haiku  # 단순 탐색, 최소 비용
model: inherit  # 부모 모델 상속
```

**모델 선택 가이드**:
- **기본값**: `opus` 사용 권장 (최고 품질 출력)
- **다운그레이드**: 비용 절감이 명확히 필요할 때만
  - `sonnet`: 단순 작업, 비용 최적화 필요 시
  - `haiku`: 빠른 응답만 필요한 단순 탐색
  - `inherit`: 부모 대화 모델을 따라야 할 때
...
3. **모델은 opus 기본**: 최고 품질 출력을 위해 opus 사용
   - 비용 절감이 명확히 필요할 때만 다운그레이드
   - `sonnet`: 단순 작업, 비용 민감한 경우
   - `haiku`: 빠른 응답만 필요한 단순 탐색
